...
void RSA::setParameters(int256_t p, int256_t q, int256_t r)
{
  ...
  {
    throw errorid=1;//std::exception("[ERROR] p or q is not a prime number!");
  }
  else if ((r < m) && (r > 1) && (Euclidean::euclidean(r, phi_of_m) != 1)) // r and phi of m must be coprime
  {
    throw  errorid=2;//std::exception("[ERROR] r is not equal or less than p * q (=> m), or r and phi of m are not coprime!");
  }
}
...
void RSA::calcKeys()
{
  ...
  {
    throw  errorid=3;//std::exception("[ERROR] Keys already calculated!");
  }
